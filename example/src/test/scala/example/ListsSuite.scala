 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of negative and positive numbers") {
    assert(sum(List(-1,-2,0,3,2,1)) === 3)
  }

  test("sum of zero filled list") {
  ...
  test("max of a descending order numbers") {
    assert(max(List(7, 3, 2)) === 7)
  }

  test("max of a ascending order numbers") {
    assert(max(List(2, 3, 7)) === 7)
  }

  test("max of a zero filled list") {
